...
def decode_lunar_year(yy: int, k: int) -> list[LunarDate]:
    ...
    if leap_month == 0:
        ...
        for mm in range(1, leap_month + 1):
            ly.append(LunarDate(1, mm, yy, False, current_jd))
            current_jd += regular_months[mm - 1]
        ly.append(LunarDate(1, leap_month, yy, True, current_jd))
...
def get_year_info(year):
    if (year < 1800 or year >= 2200):
        raise ValueError(f"Can not transform of year {year}")
    elif (year < 1900) :
    ...
    return decode_lunar_year(year, yearCode)


def find_lunar_date(jd : int, ly : list[LunarDate]) :
    i = len(ly) - 1
    while (jd < ly[i].jd) :
        i -= 1
...
def get_lunar_date(day, month, year) :
