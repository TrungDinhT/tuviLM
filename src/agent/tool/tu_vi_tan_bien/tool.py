...
from .constant import get_tu_tuc_section_id
...
def get_star_description(ctx: RunContext[TuviAgentDeps], star_name: str) -> str | SectionContent:
    ...
    section_id = get_tu_tuc_section_id(star_name)
    if section_id is None:
        return "Không tìm thấy mô tả cho sao này."
    return ctx.deps.require_book().read_section(section_id)


def get_star_role_interaction(ctx: RunContext[TuviAgentDeps], star_name: str, role: str) -> str | SectionContent:
