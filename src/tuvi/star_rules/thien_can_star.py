...
def get_thanh_long_position(locton_position: TYPE_DIA_CHI, direction: Literal[1, -1]) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 2 * direction) % 12]

def get_tieu_hao_position(locton_position: TYPE_DIA_CHI, direction: Literal[1, -1]) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 3 * direction) % 12]

def get_ln_van_tinh_position(locton_position: TYPE_DIA_CHI) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 3) % 12]

def get_tuong_quan_position(locton_position: TYPE_DIA_CHI, direction: Literal[1, -1]) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 4 * direction) % 12]

def get_tau_thu_position(locton_position: TYPE_DIA_CHI, direction: Literal[1, -1]) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 5 * direction) % 12]

def get_duong_phu_position(locton_position: TYPE_DIA_CHI) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 5) % 12]

def get_phi_liem_position(locton_position: TYPE_DIA_CHI, direction: Literal[1, -1]) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 6 * direction) % 12]

def get_hy_than_position(locton_position: TYPE_DIA_CHI, direction: Literal[1, -1]) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 7 * direction) % 12]

def get_benh_phu_position(locton_position: TYPE_DIA_CHI, direction: Literal[1, -1]) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 8 * direction) % 12]

def get_quoc_an_position(locton_position: TYPE_DIA_CHI) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 8) % 12]

def get_dai_hao_position(locton_position: TYPE_DIA_CHI, direction: Literal[1, -1]) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 9 * direction) % 12]

def get_phuc_binh_position(locton_position: TYPE_DIA_CHI, direction: Literal[1, -1]) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 10 * direction) % 12]

def get_da_la_position(locton_position: TYPE_DIA_CHI) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 11) % 12]

def get_quan_phu_position(locton_position: TYPE_DIA_CHI, direction: Literal[1, -1]) -> TYPE_DIA_CHI:
    locton_index = LIST_DIA_CHI.index(locton_position)
    return LIST_DIA_CHI[(locton_index + 11 * direction) % 12]
...
def get_star_by_thien_can_position(thien_can: TYPE_THIEN_CAN, direction: Literal[1, -1]) -> list[tuple[str, TYPE_DIA_CHI]]:
    ...
    return [
        ...
        ("Thanh Long", get_thanh_long_position(locton_position, direction)),
        ("Tiểu Hao", get_tieu_hao_position(locton_position, direction)),
        ("LN Văn Tinh", get_ln_van_tinh_position(locton_position)),
        ("Tướng Quân", get_tuong_quan_position(locton_position, direction)),
        ("Tấu Thư", get_tau_thu_position(locton_position, direction)),
        ("Đường Phù", get_duong_phu_position(locton_position)),
        ("Phi Liêm", get_phi_liem_position(locton_position, direction)),
        ("Hỷ Thần", get_hy_than_position(locton_position, direction)),
        ("Bệnh Phù", get_benh_phu_position(locton_position, direction)),
        ("Quốc Ấn", get_quoc_an_position(locton_position)),
        ("Đại Hao", get_dai_hao_position(locton_position, direction)),
        ("Phục Binh", get_phuc_binh_position(locton_position, direction)),
        ("Đà La", get_da_la_position(locton_position)),
        ("Quan Phủ", get_quan_phu_position(locton_position, direction)),
