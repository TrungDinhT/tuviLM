...
- Within that single rule set, role rules and sao rules are **named lists** (`ROLE_RULES`, then `CHINH_TINH_RULES`, `PHU_TINH_RULES`, `TU_HOA_RULES`) registered on **one** `PlacementRuleCompiler`; resolved positions are **partitioned** into typed maps that mirror `Cung.role: CungRole` vs `Cung.saos: tuple[Sao, ...]`.
...
3. **Two ontologies, one registration graph.** The single rule set is partitioned into **lists** (roles vs sao) for clarity; **one** `PlacementRuleCompiler` registers the combined graph so cross-references (sao → palace) validate and resolve without duplicating anchors.
...
flowchart TB
    ...
    Natal --> Period
    Natal --> PlacementCompiler[PlacementRuleCompiler ROLE + CHINH PHU TU_HOA]
    PlacementCompiler --> Engine[PlacementEngine]
    Engine --> FlatPos["flat ComponentId to DiaChi"]
    FlatPos --> Partition["partition by Role vs sao ids"]
    Partition --> RolePos["role_positions: Role to DiaChi"]
    Partition --> SaoPos["sao_positions: ComponentId to DiaChi"]
...
2. **Rule layer** — one rule set in [rules.py](../placement/rules.py), one engine, **one** `PlacementRuleCompiler` per natal resolve, registering **`ROLE_RULES`** then **`CHINH_TINH_RULES`**, **`PHU_TINH_RULES`**, **`TU_HOA_RULES`**. Output is split into **`role_positions`** and **`sao_positions`** by id ontology (`Role` vs non-role star ids).
...
- **Decision 6 — One `PlacementRuleCompiler` for natal, partitioned rule lists.** [rules.py](../placement/rules.py) exposes **`ROLE_RULES`** and the sao-side lists (**`CHINH_TINH_RULES`**, **`PHU_TINH_RULES`**, **`TU_HOA_RULES`**); **[bundle.py](../placement/bundle.py)** provides **`get_default_placement_rule_compiler()`**, which registers each list on **one** compiler so the dependency graph stays connected. **[natal_placement_resolver.py](../builder/natal_placement_resolver.py)** exposes **`resolve_natal_placement()`** returning **`NatalPlacement`** with **`role_positions`** / **`sao_positions`** from **partitioning** the resolved map. Catalog id coverage is enforced at chart build (Decision 7). See [workstreams/04_compiler_split_integrity.md](workstreams/04_compiler_split_integrity.md).
- **Decision 7 — Catalog vs rules, one check at `LaSo` build.** When building a `LaSo`, assert **one** invariant: every placement id registered on the **natal placement compiler** resolves in [component_catalog.py](../builder/component_catalog.py)’s JSON-backed id map (which already includes `CungRole` alongside sao / tu hoa / etc.). Call site: `LaSoBuilder.build` (workstream 05). **Not required:** every catalog JSON row must have a placement rule (unused rows are allowed). See [workstreams/05_static_chart.md](workstreams/05_static_chart.md).
- **Decision 8 — No duplicate rule sets.** Rules stay in one file, one logical set. **Lists** partition by ontology; **registration** uses one compiler instance for natal.
...
src/refactored/
  ...
  placement/
    ...
    rules.py                          (ROLE_RULES + sao lists)
    bundle.py                         (get_default_placement_rule_compiler — full graph wiring)
  ...
  builder/natal_placement_resolver.py (resolve_natal_placement, NatalPlacement)
```
...
- One `PlacementRuleCompiler` for natal via **`bundle.get_default_placement_rule_compiler()`** (same four lists); `LaSoBuilder.build` runs the catalog guard over that compiler’s registered ids.
...
- `LaSo`, `Cung` populated, `LaSoBuilder` working — `Cung.role` / `Cung.saos` from partitioned **`role_positions`** / **`sao_positions`**.
